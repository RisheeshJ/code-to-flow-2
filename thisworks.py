    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_rate_limit and attempt < max_retries - 1:
                wait = 5 * (attempt + 1)
                print(f"Rate limited on '{chunk_name}', waiting {wait}s (attempt {attempt + 1}/{max_retries})")
                time.sleep(wait)
                continue

            print(f"Error generating flowchart for '{chunk_name}': {e}")
            return f"graph TD\n    ERR[Error: {chunk_name}]"

    return f"graph TD\n    ERR[Rate limited: {chunk_name}]"
...
def process_production_code(code, lang_choice):
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if i < len(chunks) - 1:
                time.sleep(1.5)  # stay well under per-minute rate limits on big files
